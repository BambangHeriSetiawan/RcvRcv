...
class ExampleExpandableDataProvider extends AbstractExpandableDataProvider {
  private List<Pair<Items, List<Item>>> mData;

  // for undo group item
  private Pair<Items, List<Item>> mLastRemovedGroup;
  private int mLastRemovedGroupPosition = -1;

  // for undo child item
  private Item mLastRemovedChild;
  ...
  public ExampleExpandableDataProvider() {
    ...
    for (int i = 0; i < groupItems.length(); i++) {
      ...
      final ConcreteItems group = new ConcreteItems(groupId, groupText);
      ...
      for (int j = 0; j < childItems.length(); j++) {
        ...
        children.add(new ConcreteItem(childId, childText));
      }

      mData.add(new Pair<Items, List<Item>>(group, children));
    }
  }
  ...
  @Override
  public Items getGroupItem(int groupPosition) {
  ...
  public Item getChildItem(int groupPosition, int childPosition) {
    ...
    final List<Item> children = mData.get(groupPosition).second;
  ...
  public void moveGroupItem(int fromGroupPosition, int toGroupPosition) {
    ...
    final Pair<Items, List<Item>> item = mData.remove(fromGroupPosition);
  ...
  public void moveChildItem(int fromGroupPosition, int fromChildPosition, int toGroupPosition, int toChildPosition) {
    ...
    final Pair<Items, List<Item>> fromGroup = mData.get(fromGroupPosition);
    final Pair<Items, List<Item>> toGroup = mData.get(toGroupPosition);

    final ConcreteItem item = (ConcreteItem) fromGroup.second.remove(fromChildPosition);

    if (toGroupPosition != fromGroupPosition) {
      // assign a new ID
      final long newId = ((ConcreteItems) toGroup.first).generateNewChildId();
  ...
  private long undoChildRemoval() {
    Pair<Items, List<Item>> group = null;
    int groupPosition = -1;
  ...
  public static final class ConcreteItems extends Items {
    ...
    ConcreteItems(long id, String text) {
      mId = id;
      mText = text;
      mNextChildId = 0;
    }
  ...
  public static final class ConcreteItem extends Item {

    private long mId;
    private final String mText;
    private boolean mPinned;

    ConcreteItem(long id, String text) {
      mId = id;
      mText = text;
    }
